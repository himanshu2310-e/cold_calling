...
import {
  ArrowLeft, Phone, Mail, Building2, Globe,
  LinkIcon, MapPin, Tag, Star, Edit, Trash2,
  Clock, User, CalendarClock, Mic, FileText,
...
export default function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: timelineData } = useQuery({
    ...
    queryFn: () => callService.getTimeline(id),
  });

  const { data: recordingsData } = useQuery({
    queryKey: ['recordings', id],
    queryFn: () => callService.getRecordingsByLead(id),
  });

  const { data: notesData } = useQuery({
    queryKey: ['notes', id],
    queryFn: () => noteService.getNotesByLead(id),
  });
    ...
    return (
      <div className="p-6 lg:p-8 max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-2 h-96 rounded-2xl bg-zinc-800/30 animate-pulse" />
          <div className="h-96 rounded-2xl bg-zinc-800/30 animate-pulse" />
        </div>
      </div>
    );
  }
  ...
  if (!lead) {
    return (
      <div className="p-6 lg:p-8 max-w-[1600px] mx-auto text-center py-20">
        ...
        <p className="text-xs text-zinc-500 mb-4">This lead may have been deleted or does not exist.</p>
        <button onClick={() => router.push('/admin/leads')} className="btn-primary cursor-pointer">
          Back to Leads
        </button>
      </div>
    );
  ...
  const priorityConfig = LEAD_PRIORITY_CONFIG[lead.priority] || { label: lead.priority, color: '#A1A1AA', bg: 'rgba(161,161,170,0.12)' };
    ...
    <div className="p-6 lg:p-8 max-w-[1600px] mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleToggleFavorite} className="btn-secondary cursor-pointer" title="Favorite">
            <Star className="w-4 h-4" style={{ color: lead.isFavorite ? '#F59E0B' : undefined, fill: lead.isFavorite ? '#F59E0B' : 'none' }} />
          </button>
          <button onClick={() => setEditOpen(true)} className="btn-secondary flex items-center gap-1.5 cursor-pointer">
            <Edit className="w-3.5 h-3.5" />
            <span>Edit</span>
          </button>
          <button onClick={handleDelete} className="btn-secondary text-red-400 hover:text-red-300 hover:border-red-500/30 cursor-pointer">
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 text-xl font-bold flex-shrink-0">
            {lead.fullName[0]}
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-bold text-white tracking-tight">{lead.fullName}</h1>
            <div className="flex flex-wrap items-center gap-3 mt-2">
              <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider" style={{ color: statusConfig.color, background: statusConfig.bg }}>
                {statusConfig.label}
              </span>
              <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider" style={{ color: priorityConfig.color, background: priorityConfig.bg }}>
                {priorityConfig.label} Priority
              </span>
              {lead.tags?.map((tag) => (
                <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold text-zinc-400 bg-zinc-800 border border-zinc-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 rounded-2xl border p-5 space-y-4" style={{ background: '#141418', borderColor: '#1E1E22' }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              {[
                ...
                { icon: LinkIcon, label: 'LinkedIn', value: lead.linkedin || '—' },
                { icon: MapPin, label: 'Location', value: [lead.city, lead.state, lead.country].filter(Boolean).join(', ') || '—' },
                { icon: FileText, label: 'Industry', value: lead.industry || '—' },
                ...
                <div key={item.label} className="flex items-start gap-3 p-3 rounded-xl bg-zinc-900/50">
                  ...
                  <div>
                    ...
                    <span className="text-zinc-300 mt-0.5 block break-all">{item.value}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border p-5 space-y-3" style={{ background: '#141418', borderColor: '#1E1E22' }}>
              <h3 className="text-sm font-bold text-white">Assignment</h3>
              <div className="flex items-center gap-3 p-3 rounded-xl bg-zinc-900/50">
                <div className="w-8 h-8 rounded-full bg-purple-500/10 border border-purple-500/20 flex items-center justify-center text-purple-400 text-[10px] font-bold">
                  {agent ? agent.firstName[0] : '?'}
                </div>
                <div>
                  <p className="text-xs font-semibold text-white">{agent ? `${agent.firstName} ${agent.lastName}` : 'Unassigned'}</p>
            ...
            <div className="rounded-2xl border p-5 space-y-3" style={{ background: '#141418', borderColor: '#1E1E22' }}>
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="text-zinc-300 font-mono">{format(new Date(lead.createdAt), 'MMM d, yyyy')}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'timeline' && (
        <div className="rounded-2xl border p-5" style={{ background: '#141418', borderColor: '#1E1E22' }}>
          <h3 className="text-sm font-bold text-white mb-4">Call Timeline</h3>
          {timeline.length === 0 ? (
            <div className="py-12 text-center">
              <Clock className="w-10 h-10 text-zinc-700 mx-auto mb-2" />
              <p className="text-xs text-zinc-500">No call history for this lead yet</p>
            </div>
          ) : (
            <div className="space-y-3">
              {timeline.map((entry: any) => {
                ...
                return (
                  <div key={entry._id} className="flex items-start gap-3 p-3 rounded-xl bg-zinc-900/50 hover:bg-zinc-800/50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <p className="text-xs font-semibold text-white">{entry.outcome || 'Call'}</p>
                        <div className="flex items-center gap-3">
                          ...
                          {callRecording ? (
                            <button
                              ...
                              className={`flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider transition-colors cursor-pointer ${
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-zinc-400 transition-colors cursor-pointer"
                            >
                              <Upload className="w-2.5 h-2.5" />
                              <span>Upload Rec</span>
                            </button>
                          )}
                        </div>
                      </div>
                      {entry.notes && <p className="text-[10px] text-zinc-500 mt-1">{entry.notes}</p>}
                      <p className="text-[9px] text-zinc-650 mt-1 font-mono">
