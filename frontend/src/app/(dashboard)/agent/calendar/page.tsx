// ============================================
// Premium Notion / Motion Style Calendar Redesign (Agent Version)
// ============================================
'use client';
...
export default function AgentCalendarPage() {
  ...
  // Fetch agent's follow-ups (backend automatically filters agent role follow-ups)
  const { data: followupsData } = useQuery({
    queryKey: ['followups-cal'],
    queryFn: () => followupService.getFollowUps(),
  });
  const agentFollowUps = useMemo(() => followupsData?.data || [], [followupsData]);

  // Fetch leads for dropdown select options
  ...
  const getEventsForDay = (day: Date) => {
    return agentFollowUps.filter((fu: any) => fu.scheduledDate && isSameDay(new Date(fu.scheduledDate), day));
  };

  // Filtered Events
  const filteredEvents = useMemo(() => {
    return agentFollowUps.filter((fu: any) => {
  ...
  }, [agentFollowUps, searchQuery, priorityFilter, statusFilter]);

  // Statistics counters
  const stats = useMemo(() => {
    const total = agentFollowUps.length;
    const completed = agentFollowUps.filter((f: any) => f.isCompleted).length;
    ...
    <div className="p-6 lg:p-8 max-w-[1600px] mx-auto space-y-6 text-white min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-800 pb-5">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1">Schedule and manage your follow-ups</p>
        </div>
